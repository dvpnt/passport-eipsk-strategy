...
function Strategy(options, verify) {
	options.authorizationURL = options.authorizationURL ||
		'https://pro.culture.ru/auth/oauth2/authorize';
	options.tokenURL = options.tokenURL ||
		'https://pro.culture.ru/auth/oauth2/token';

	BaseStrategy.call(this, options, verify);

	this.name = 'eipsk';
	this._profileURL = options.profileURL || 'https://pro.culture.ru/auth/api/1.0/user';
}
...
Strategy.prototype.parseProfile = function(json) {
	...
	profile.profileUrl = 'https://pro.culture.ru/cabinet/users/' + profile.id;
